...
import comments from './comments.vue'
  export default {
    ...
    created() {
       Echo.channel('episode.'+this.$route.params.id)
    .listen('CommentEvent', (e) => { 
           let comment  = 
              {
            avatar:this.avatar(),
            title: e.data.name,
            subtitle: e.data.content,
           }
    ...
    this.$store.dispatch('DetialEpisode',this.$route.params.id)
    .then((res)=>{
...
this.$toaster.error('something happened error try later.');
    ...
    this.$store.dispatch('getCommentsEpisode',{params:"?id="+this.$route.params.id+"&type=ee"})
    .then((res)=>{
...
this.$toaster.error('something happened error try later.');
    ...
    methods:{
        avatar(){
          return this.$store.getters.user != 'undifned'?this.$store.getters.avatarUser:this.$store.getters.avatarPublic;
        }
    },
    data () {
      return {
